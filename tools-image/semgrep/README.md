...
The tools image applies four narrow corrections to the rule bundle pinned by
...
Semgrep 1.177.0 scans the four corrected vendored paths, and the verifier
...
expressions, and two invalid returns. Private modes, standard subprocess result
and exception constructors, the exact pull-request base SHA, and all lambda
forms must remain clean. The other rule in `return.yaml` remains in the result
set as an additional byte-pinned control.
