...
cheap "scout", builds a risk-tuned plan, dispatches the native review-agent fleet
(`code-reviewer` / `security-reviewer` / `test-reviewer`, Opus + Sonnet) in parallel
across cohesive file groups, and — optionally — grounds findings with real static-analysis
tools from a Docker container. It synthesizes everything into a `CodeReviewReport`
(terminal markdown summary + JSON artifact) with standards citations and CI gating.
...
- `prompts/` — `scout.md` (surface profiling + tool selection), `lenses.md` (9-lens catalog).

## Key design decisions (don't relitigate without reason)
- **Delegate to the native fleet** rather than self-orchestrated persona subagents.
