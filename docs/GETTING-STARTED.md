...
Two pages for a person deciding whether to run this on a real codebase: what it does, what it costs, how to run it, and what it will not do. The complete contract is in [`PANOPTICON.md`](PANOPTICON.md); this page is the map.
...
2. **Scout.** One cheap agent per group profiles the files and picks the review domains that apply, on top of a floor that always applies (code, data, tests, architecture, and security where there is a security surface).
3. **Fan-out.** One reviewer agent per (domain, group) cell, run in parallel, each inside a registered shell that limits its tools to reading and returning JSON. Reviewers cite the OCRDb catalog, a fixed taxonomy of review findings, so two runs on two codebases speak the same language.
4. **Tools.** If the `panopticon-tools` Docker image is present, real scanners run against the same tree (Bandit, Semgrep, gitleaks, osv-scanner, Trivy, ESLint security, and language-specific auditors). Their findings enter the same queue as the agents' findings.
5. **Verify.** Every finding, agent or tool, goes to an independent advisor that reads only the cited files and returns confirmed, rejected, or needs-more-info. Grades and the gate count confirmed findings only, unless you opt in to unverified ones.
...
One real measurement, Panopticon reviewing its own repository on 2026-09-20 in its most expensive configuration (adversarial security mode, all tools, Opus-tier advisors):
...
- **Scope.** `-c` (this branch against its base), `--pr N`, `-d dir` and `-f file` review a slice. A 25-file PR still queues every finding for verification, so pair a delta review with `--max-verify N` (about twice the changed-file count is a good default).
...
Prerequisites: Python 3.11+, `pyyaml` and `jsonschema`, an agent host on `PATH`, and optionally Docker with the `panopticon-tools` image (pull it if you have access to the package; otherwise `docker build -t panopticon-tools .` from the checkout, which takes over an hour).
...
Readiness writes nothing and launches nothing. It prints one table: the guide's path, the sub-skills it found, the committed matrix, any run waiting to resume, which host CLIs are on `PATH`, the Docker daemon and image, and the last measured host posture. Every failing row carries its remedy. Fix what gates, then:
...
- `.panopticon/<run-tag>-report.json.html`: the same report as one self-contained page for humans. Attach it, mail it, drop it in a shared folder.
- `.panopticon/runs/<run-tag>/`: the working set (findings, verdicts, coverage, tool output, dispatch ledger). Safe to delete after the run; the reports above survive.

Exit codes: `0` clean, `1` gate failed, `2` inconclusive, `3` unreadable catalog bundle, `4` an artifact failed its own schema.

## What to trust in the report

Read these lines before the findings:

- **Coverage certified** or **NOT CERTIFIED** with the reason. Certified means every planned cell returned, every tool the scout requested produced output, and no integrity check fired.
...
- **Evidence status** per finding: `tool_confirmed`, `advisor_confirmed`, `corroborated`, `needs_more_info`, `unverified`, `rejected`. Severity is what the reviewer claimed; evidence is what the pipeline could verify. They are independent axes, and only confirmed evidence grades or gates by default.
- **Discarded claims** are kept in a sibling file, not deleted. A high rejection rate on the tool axis is normal (most scanner hits on a mature codebase are noise); a high rejection rate on the agent axis is a signal about the reviewer tier.
...
- It does not run as a service, does not collect telemetry, and does not need network access beyond your agent host (and the optional EPSS and dependency-database lookups you enable).
