...
- **generic host (portable):** the same contract without the Workflow — a nested per-group sub-orchestrator (or per-entry dispatch) holding scoped Write, dispatching pending-only, returning a tally. Any non-Claude host (Kimi, Codex, a bare CLI session) uses this path today — `driver run --host` accepts `claude`, `generic`, or `gemini`. There is deliberately **no `--host codex`** on the driver: `--emit-host-agents codex` registers Codex's 5.x role shells, and a Codex session then drives the `generic` path above. (#run10: the 4.x `codex_runner.py` / `--emit-kimi-swarm` execution adapters were retired with the `panel_review`/`lens_sweep` roles; Kimi and Codex adapters are being rebuilt against the 5.x matrix for 5.2.)
