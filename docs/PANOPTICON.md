...
- Dispatch one agent per entry: `enforced` → `subagent_type: entry["agent"]` (a registered `panopticon-*` shell, tools+model host-enforced); else general-purpose with `entry["prompt"]` and the model named by `entry["model"]` (omit when null). Each reviewer/advisor **self-writes** its own `entry["out_file"]` (a findings file for review, a verdict bundle for verify) and returns a one-line confirmation — findings/verdicts never transit the controller. **Exception — `entry["delivery"] == "return_json"`:** the entry is **return-persist** (the read-only `panopticon-advisor` per-finding tool-advisor, which has no Write), so it RETURNS its verdict JSON and the controller writes it to `entry["out_file"]` (`verdicts/<queue_id>.json`) after confirming it parses — exactly like the scout, not a self-write.
...
- Uninstall the guard after the fan-out (`write_guard_hook.uninstall()`, or `uninstall(plan=req["entries"])` to drop only THIS fan-out's paths and leave the guard armed for any still-active one — #11). **The terminal `complete` status carries a `teardown` field naming this call** — the run is over at that point, so no fan-out still needs write access, and a guard left armed denies every LATER Write/Edit in the session (the operator's included), since the allowlist is the complete set of permitted writes. `uninstall()` is safe to call unconditionally; `write_guard_hook.is_armed()` returns `(armed, grants)` if you want to assert the state instead — it is **fail-closed while registered**: an absent, unreadable, or malformed allowlist DENIES guarded writes with a loud reason instead of silently allowing them.
