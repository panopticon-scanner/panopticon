...
- Uninstall the guard after the fan-out (`write_guard_hook.uninstall()`, or `uninstall(plan=req["entries"])` to drop only THIS fan-out's paths and leave the guard armed for any still-active one — #11) — it is **fail-closed while registered**: an absent, unreadable, or malformed allowlist DENIES guarded writes with a loud reason instead of silently allowing them.
