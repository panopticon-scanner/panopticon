...
- **generic host (portable):** the same contract without the Workflow — a nested per-group sub-orchestrator (or per-entry dispatch) holding scoped Write, dispatching pending-only, returning a tally. Any non-Claude host (Kimi, Codex, a bare CLI session) uses this path today — `driver run --host` accepts `claude`, `generic`, or `gemini`. There is deliberately **no `--host codex`** on the driver: Codex is a *dispatch-layer* host (`dispatch.py --host codex` renders its prompts, `--emit-host-agents codex` registers its shells, `codex_runner.py` executes them), and its runs go through the `generic` driver path above.
