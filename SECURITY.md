...
The reviewed tree is treated as attacker-controlled. Reviewers read it under confinement, scanners run in a container with the tree mounted read-only, git is invoked with hooks, filters and external drivers disabled, and a PR under review cannot replace the operator's review configuration. The "Notes" section of [`docs/PANOPTICON.md`](docs/PANOPTICON.md) lists these controls and their known limits; a gap between what that section claims and what the code does is a vulnerability under this policy.
