...
The reviewed tree is treated as attacker-controlled. Reviewers read it under confinement on every host that proves a read guard (and the report says when one does not), scanners run in a container with the tree mounted read-only, git is invoked with hooks, filters and external drivers disabled, and a PR under review cannot replace the operator's review configuration. The user guide describes the reviewer confinement and its refusals in [`docs/guide/notes.md`](docs/guide/notes.md), the PR-worktree rule under "Modes" in [`docs/PANOPTICON.md`](docs/PANOPTICON.md), and the five measured host controls in [`docs/guide/host-capabilities.md`](docs/guide/host-capabilities.md); the read-only container mount lives in `skill/scripts/run_tools.py` and the git hardening in `skill/scripts/safe_git.py`, each with its rationale in the module docstring. A gap between what any of those claims and what the code does is a vulnerability under this policy.
