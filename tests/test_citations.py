...
class TestCatalog(unittest.TestCase):
    def test_loads_and_has_core_entries(self):
        ...
        self.assertEqual(cat["cwe"]["CWE-89"], "Improper Neutralization of Special Elements used in an SQL Command ('SQL Injection')")
...
class TestEpss(unittest.TestCase):
    def test_lookup_success_and_cache(self):
        ...
        def opener(url, timeout=0):
            calls["n"] += 1
            return _FakeResp({"data": [{"cve": "CVE-2023-1234",
                                        "epss": "0.42", "percentile": "0.97",
                                        "date": "2026-07-20"}]})
    ...
    def test_network_error_omits_cve(self):
        def opener(url, timeout=0):
    ...
    def test_lookup_accepts_lowercase_cve(self):
        def opener(req, timeout=0):
            return _FakeResp({"data": [{"cve": "CVE-2023-1234", "epss": "0.42",
                                        "percentile": "0.97", "date": "2026-07-20"}]})
        with tempfile.TemporaryDirectory() as d:
            out = cit.epss_lookup(["cve-2023-1234"], os.path.join(d, "c.json"), opener=opener)
    ...
    def test_request_capped_read_and_user_agent(self):
        ...
        def opener(req, timeout=0):
            holder["req"] = req
            resp = _FakeResp({"data": [{"cve": "CVE-2023-1234", "epss": "0.1",
                                        "percentile": "0.5", "date": "2026-07-20"}]})
            holder["resp"] = resp
            return resp
        with tempfile.TemporaryDirectory() as d:
...
class TestEnrich(unittest.TestCase):
    ...
    def test_enrich_full_finding(self):
        f = {"source": "agent:security-reviewer", "citations": {
            "cwe": ["CWE-89", "bogus"],
            "ssvc": {"inputs": {"exploitation": "active", "exposure": "open", "impact": "high"}}}}
    ...
    def test_enrich_epss_enabled_end_to_end_adds_epss_citation(self):
        ...
        f = {"source": "tool:trivy", "citations": {"cve": ["CVE-2023-1234"]}}
        with tempfile.TemporaryDirectory() as d:
            cit.enrich_citations([f], self.cat, epss_enabled=True,
