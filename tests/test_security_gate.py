...
class TestEveryNameBasedDropReachesTheRedteamGate(unittest.TestCase):
    ...
    What must still be silent is the scanner's own `.panopticon/` artifacts and
    generated bytecode, where re-gating a previous run's own discarded report is
    the compounding noise run-10 D1 removed. This docstring used to name "a
    `pyvenv.cfg` beside the tree" in that list, and #1839 ruling 2 (run-14
    SEC-1486247143) takes it out: the marker is a file the TARGET wrote, so
    #1740's reason for disclosing a directory NAME applies to it more strongly,
    not less -- and the planted form of it (a bare `pyvenv.cfg` with no
    environment under it) is not a virtualenv at all.
    """

    def _repo(self, root, uri, marker_dir=None, nested=False, shape=True):
        ...
        if marker_dir:
            ...
            if shape:
                # #1839: a marker alone is a claim the target can commit in one
                # file. A real virtualenv also carries an interpreter.
                os.makedirs(os.path.join(root, marker_dir, "bin"), exist_ok=True)
                with open(os.path.join(root, marker_dir, "bin", "python"), "w") as fh:
                    fh.write("")
        with open(os.path.join(tools, "semgrep.sarif"), "w", encoding="utf-8") as fh:
    ...
    def test_a_marker_confirmed_virtualenv_reaches_the_redteam_gate(self):
        # #1839 ruling 2, replacing `test_a_pyvenv_cfg_beside_it_keeps_the_drop
        # _silent`: the marker used to make the drop silent in BOTH modes, so
        # `high` and `suppressed` were empty and a payload under a target-marked
        # directory passed a redteam gate outright -- the same hole #1740 closed
        # for `app/venv/` by NAME, one step further on.
        for mode, gated in (("standard", 0), ("redteam", 1)):
            with tempfile.TemporaryDirectory() as root:
                ...
                findings, _d, _failures, high, suppressed = gate.evaluate(
                    tools, manifest, security_mode=mode)
            self.assertEqual(findings, [], mode)          # never in the body
            self.assertEqual(len(high), gated, mode)
            self.assertEqual([f["suppressed"] for f in suppressed],
                             [ingest_tools.MARKER_VENV_PREFIX + "app/venv"], mode)
        # ...and the verdict line stops calling this class a directory NAME.
        buf = io.StringIO()
        with tempfile.TemporaryDirectory() as root:
            tools, manifest = self._repo(
                root, "app/venv/patched_auth.py", marker_dir="app/venv", nested=True)
            with contextlib.redirect_stdout(buf), \
                    contextlib.redirect_stderr(io.StringIO()):
                gate.main(["--tools-dir", tools, "--manifest", manifest])
        self.assertIn("1 suppressed by directory name or marker", buf.getvalue())
        self.assertIn("virtualenv-by-marker (pyvenv.cfg:app/venv: 1)", buf.getvalue())

    def test_a_bare_marker_is_not_a_virtualenv_at_the_gate_either(self):
        # #1839 rulings 1 and 3: a `pyvenv.cfg` with no environment under it is
        # one committed file. Under a name no convention reserves, it prunes
        # nothing -- the finding is in the BODY, in both modes, and gates like
        # any other HIGH rather than being suppressed-and-re-admitted.
        for mode in ("standard", "redteam"):
            with tempfile.TemporaryDirectory() as root:
                tools, manifest = self._repo(
                    root, "app/installed/patched_auth.py",
                    marker_dir="app/installed", nested=True, shape=False)
                findings, _d, _failures, high, suppressed = gate.evaluate(
                    tools, manifest, security_mode=mode)
    ...
    def test_the_marker_case_is_not_vacuous(self):
        # MUTATION: the same layout WITHOUT any marker must still gate under
        # redteam (on the NAME, #1740), so the tests above pin what they say
        # they pin -- the marker and its shape, not the nesting.
        with tempfile.TemporaryDirectory() as root:
            tools, manifest = self._repo(root, "app/venv/patched_auth.py",
                                         nested=True)
            _f, _d, _failures, high, suppressed = gate.evaluate(
                tools, manifest, security_mode="redteam")
        self.assertEqual(len(high), 1)
        self.assertEqual([f["suppressed"] for f in suppressed], ["venv"])
