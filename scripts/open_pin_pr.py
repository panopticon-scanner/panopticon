...
# The checked-out repository supplies this audited resolver, but it is never
# searched for the git/gh executable. Match the import convention in
# scripts/file_issues.py when this file is launched directly by Actions.
_SKILL_SCRIPTS = Path(__file__).resolve().parents[1] / "skill" / "scripts"
if str(_SKILL_SCRIPTS) not in sys.path:
    sys.path.insert(0, str(_SKILL_SCRIPTS))
from executable import resolve  # noqa: E402


TRUSTED_PATH = "/usr/local/bin:/usr/bin:/bin:/opt/homebrew/bin"
PROCESS_TIMEOUT = 120
_SHA = re.compile(r"[0-9a-f]{40}\Z")
_BRANCH = re.compile(r"chore/bump-(rustup|trivy|rust-toolchain)-([0-9]+\.[0-9]+\.[0-9]+)\Z")
...
def _allowed_command(command: list[str]) -> bool:
    """Only argv shapes this one pin workflow emits; data remains separate."""
    if not isinstance(command, list) or not all(isinstance(a, str) and a for a in command):
        return False
    if command[:1] == ["git"]:
        args = command[1:]
        if args in (["rev-parse", "--is-shallow-repository"],
                    ["rev-parse", "FETCH_HEAD"], ["rev-parse", "FETCH_HEAD^"],
                    ["rev-parse", "HEAD"],
                    ...
                    ["add", "--", "Dockerfile"],
                    ["show", "FETCH_HEAD:Dockerfile"]):
            return True
        if (len(args) == 4 and args[:3] == ["ls-remote", "--heads", "origin"]
                and args[3].startswith("refs/heads/")
                and _BRANCH.fullmatch(args[3][len("refs/heads/"):])):
            return True
        if (args[:2] == ["fetch", "--no-tags"]
                and args[2:] in (["origin", "refs/heads/main"],
                                 ["--unshallow", "origin", "refs/heads/main"])):
            return True
        if (len(args) == 4 and args[:3] == ["fetch", "--no-tags", "origin"]
                and args[3].startswith("refs/heads/")
                and _BRANCH.fullmatch(args[3][len("refs/heads/"):])):
            return True
        if len(args) == 2 and args[0] == "show" and re.fullmatch(
                r"[0-9a-f]{40}:Dockerfile", args[1]):
            return True
        if len(args) == 3 and args[:2] == ["checkout", "-b"]:
            return bool(_BRANCH.fullmatch(args[2]))
        if len(args) == 3 and args[:2] == ["push", "origin"]:
            return (args[2].startswith("HEAD:refs/heads/")
                    and bool(_BRANCH.fullmatch(args[2][len("HEAD:refs/heads/"):])))
        if len(args) == 5 and args[:4] == ["rev-list", "--parents", "-n", "1"]:
            return bool(_SHA.fullmatch(args[4]))
        if (len(args) == 2 and args[0] == "rev-list"
                and re.fullmatch(r"[0-9a-f]{40}\.\.[0-9a-f]{40}", args[1])):
            return True
        if len(args) == 4 and args[:2] == ["merge-base", "--is-ancestor"]:
            return bool(_SHA.fullmatch(args[2]) and _SHA.fullmatch(args[3]))
        if len(args) == 4 and args[:2] == ["diff", "--name-only"]:
            return bool(_SHA.fullmatch(args[2]) and _SHA.fullmatch(args[3]))
        if len(args) == 5 and args[:2] == ["commit", "-m"] and args[3] == "-m":
            for family, (_version_arg, _digest_args, title, body) in FAMILIES.items():
                match = _BRANCH.fullmatch("chore/bump-" + family + "-" + args[2].rsplit(" ", 1)[-1])
                if (match and args[2] == title.format(version=match.group(2))
                        and args[4] == "Generated by the pin-freshness workflow. " + body):
                    return True
        return False
    if command[:2] == ["gh", "api"] and len(command) == 3:
        path, sep, query = command[2].partition("?")
        match = re.fullmatch(r"repos/([A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+)/pulls", path)
        ...
        heads = params.get("head", [])
        return (set(params) == {"state", "base", "head", "per_page"}
                and params["state"] == ["all"] and params["base"] == ["main"]
                and params["per_page"] == ["100"] and len(heads) == 1
                and heads[0].startswith(owner + ":")
                and bool(_BRANCH.fullmatch(heads[0][len(owner) + 1:])))
    if command[:3] == ["gh", "pr", "create"] and len(command) == 15:
        if command[3:4] != ["--repo"] or not _REPO.fullmatch(command[4]):
            return False
        if command[5:8] != ["--base", "main", "--head"]:
            return False
        branch = _BRANCH.fullmatch(command[8])
        if (not branch or command[9:10] != ["--title"]
                or command[11:12] != ["--body"]
                or command[13:15] != ["--label", "dependencies"]):
            return False
        _arg, _digests, title, body = FAMILIES[branch.group(1)]
        return (command[10] == title.format(version=branch.group(2))
                and command[12] == body)
    return False


def _run(command: list[str], cwd: Path) -> subprocess.CompletedProcess[str]:
    """Run only this workflow's git/gh commands with trusted tools and auth.

    The argv comes from the helper's validated pin metadata. Even a mistaken
    new call site cannot turn this into a generic process runner; the child
    gets no ambient PATH or interpreter/loader startup hooks. Actions checkout
    stores Git push auth in local Git config, while gh receives GH_TOKEN.
    """
    if not _allowed_command(command):
        raise RuntimeError("unexpected pin executable or arguments")
    program = command[0]
    selected = resolve(program, cwd, path=TRUSTED_PATH)
    home = os.path.expanduser("~")
    env = {"HOME": home, "PATH": selected.path_env, "LC_ALL": "C",
           "GIT_TERMINAL_PROMPT": "0"}
    if program == "gh":
        if os.environ.get("GH_TOKEN"):
            env["GH_TOKEN"] = os.environ["GH_TOKEN"]
        elif os.environ.get("GH_CONFIG_DIR"):
            env["GH_CONFIG_DIR"] = os.environ["GH_CONFIG_DIR"]
    # Same bounded, injectable subprocess.run partial pattern used by
    # scripts/file_issues.py; the resolved executable is absolute and the
    # environment is built above rather than inherited from the checkout.
    runner = functools.partial(subprocess.run, cwd=cwd, env=env,
                               timeout=PROCESS_TIMEOUT, text=True,
                               capture_output=True, check=False)
    try:
        return runner([selected.path, *command[1:]])
    except subprocess.TimeoutExpired:
        raise RuntimeError("%s timed out" % program) from None
