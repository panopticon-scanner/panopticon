...
def agent_finding(raw, path="<agent payload>"):
    """Remove controller-owned fields before an agent finding is consumed.

    All private keys belong to the controller, including future carriers.
    `_panopticon` is an ENVELOPE stamp, checked by the transport; a finding's
    own copy confers nothing. The envelope itself is never modified here.
    Public finding ids are regenerated by both normalized loaders below.
    """
    clean = dict(raw)
    for key in raw:
        if str(key).startswith("_") or key in AGENT_FORBIDDEN_FIELDS:
    ...
    for path in paths:
        ...
        for f in findings:
            if not isinstance(f, dict):
                continue
            nf = normalize_finding(agent_finding(f, path))
...
# rationale as corroborate._is_tool_sourced (#688: local copies of shared
# definitions drift).
