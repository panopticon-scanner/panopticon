...
- **`tools`** — deterministic, not discretionary: runs
  ...
  **`--no-tools` is the non-destructive rescue** when the environment will not come back: it is the
  ...
  counted in DIRECTORIES rather than findings because the scanners never entered the tree. The
  ingest side of the marker rule moved with it (#1839, the half the driver's own report reads): a
  finding under a marker-confirmed virtualenv is dropped at ANY depth as before, but it is now
  counted under a `pyvenv.cfg:<dir>` key of that fourth class and handed back for a redteam gate to
  re-admit, and a bare `pyvenv.cfg` with no interpreter or `site-packages` under it prunes NOTHING
  at either stage — one predicate, `run_tools.has_venv_shape`, answers for the scan and the ingest.
  An operator's own `--tools-exclude`/`--exclude` glob outranks it, like the name rules. A prune
  that rests on evidence nobody disputes — a nested `.worktrees` checkout, `.git`, panopticon's own
  `.panopticon/` artifacts, generated bytecode — stays silent in both modes, and
