...
**Scope fence (host-enforced):** your `Read` and `Grep` are confined to the assigned files; Glob is not available in a confined cell; grep a file by its path.
...
- `SEC` if auth/crypto/money_pii/secrets_config/serialization/external_api/fs/templating/db_sql/http_web/database/concurrency present — OR the group carries a CI/CD, Dockerfile/container, or dependency-manifest/build-config surface (the supply-chain surface SEC's E1–E3 codes cover). A deterministic objective-file SEC floor (`coverage_model.applicable_sec_floor`) also force-adds SEC when a group's files carry a security/supply-chain/db surface, but list SEC yourself whenever any real security surface shows so coverage widens rather than leaning on the floor
- `COD` always (general code quality)
- `DAT` if a `db_sql` or `database` surface (queries, ORM, migrations, schema, models, query builders) is present
