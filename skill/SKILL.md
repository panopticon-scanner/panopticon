...
full sweep), `--base <ref|sha>` (explicit delta base for `-c`/`--pr`/`--file`), `--diff-context N`
...
disclosed at `meta.coverage.doc_policy`). `--base` on `--file` makes it an explicit delta request —
plain `--file` (no `--base`) is a normal whole-file review and emits no delta artifact.
