...
- `--pr <n>` — review a GitHub PR in an isolated disposable `git worktree` (never touches your checkout). The orchestrator fetches the PR head, detects its base branch via `gh`, records the worktree path in `groups.json`'s `worktree` field, and **stages both `groups.json` and `diff-hunks.json` into `<worktree>/.panopticon/` automatically (#955) — the worktree is pipeline-ready on exit**; run scout/fan-out/synthesize with that worktree as cwd, then remove it with `diff_map.release_worktree(<path>)`. Because the worktree is a clean checkout at the PR head, a PR review is committed-only (reproducible). Same loud-fail on an unresolvable base.
- `-e` / `--explore` — discover and catalog groups; no panels unless asked.
- `--setup` — first-run provisioning + readiness check (#485): seeds a committable `.panopticon/groups.yml` from the repo's directory spine (never clobbers an existing one), scaffolds `.panopticon/` + `.gitignore` entries + `config.json` (the #486 gh-account field), then prints a readiness checklist — **READY**, or each gap with its exact fix (docker/tools-image, target-root sanity, NVD_API_KEY, registered enforced shells, groups-manifest health) and exit 1.
